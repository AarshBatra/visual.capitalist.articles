...
## Story 7: NCT of Delhi Blocks Monthly Population Weighted Average PM2.5 from 1998 to 2023
...
#### addional calculations code[not for main plot, but for blog]-----------------------

### annual average pop weighted
summary_data <- delhi_monthly_blk_data_1998_2023 %>% 
  group_by(year, subdistrict_name) %>% 
  summarise(mean_pm2.5 = mean(avg_pm2.5, na.rm = TRUE), pop_blk = subdistrict_population[1]) %>%
  ungroup() %>%
  group_by(year) %>%
  mutate(pop_wt = pop_blk/sum(pop_blk, na.rm = TRUE), 
         tot_pop = sum(pop_blk, na.rm = TRUE), 
         pop_wt_pm2.5 = mean_pm2.5*pop_wt) %>%
  summarise(avg_pm2.5_fin = sum(pop_wt_pm2.5, na.rm = TRUE))

### summer months average
summary_data <- delhi_monthly_blk_data_1998_2023 %>% 
  filter(month %in% c("march", "april", "may", "june", "july", "august", "september")) %>%
  group_by(year, subdistrict_name) %>% 
  summarise(mean_pm2.5 = mean(avg_pm2.5, na.rm = TRUE), pop_blk = subdistrict_population[1]) %>%
  ungroup() %>%
  group_by(year) %>%
...
### winter months average
summary_data <- delhi_monthly_blk_data_1998_2023 %>% 
  filter(month %in% c("october", "november", "december", "january", "february")) %>%
  group_by(year, subdistrict_name) %>% 
  summarise(mean_pm2.5 = mean(avg_pm2.5, na.rm = TRUE), pop_blk = subdistrict_population[1]) %>%
  ungroup() %>%
  group_by(year) %>%
...
### months greater than 100 micrograms per cubice meter
delhi_monthly_blk_data_1998_2023 %>%
  filter(avg_pm2.5 > 180, year == 2023)

 delhi_monthly_blk_data_1998_2023 %>%
  filter(avg_pm2.5 > 120, year == 2023) %>%
    select(month) %>%
    unique()

## 5 in 2023, 2 in 1998

### months less than 35 micrograms per cubice meter
  delhi_monthly_blk_data_1998_2023 %>%
  filter(avg_pm2.5 < 35, year == 1998) %>%
    select(month) %>%
    unique()
...
## Story 6: OpenAQ Open Air Quality Data Global Landscape 2024 report, 4 category plot
```{r story6}
#### read data
open_aq_gl_2024_data <- readxl::read_xlsx(paste0(here(), "/6.open.aq.2024.rep.country.wise.aq.status.map/raw.data/raw.data.xlsz"))
...
## Story 5: geofacet plot of India state level air pollution (1998-2022)

```{r story5}
#### load raw data
ind_bl_lev_pol_wide <- read_csv(paste0(here(), "/5.ind.st.lev.pol.timeseries/raw.data/ind_block_level_pop_weighted_pol_1998_2022.csv"))
...
## Story 4: choropleth categorical plot of India block level air pollution
